...
public class UserUnlock extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// Get login email from user.jsp
		HttpSession session = request.getSession();
		String user_email = (String)session.getAttribute("user_email");

		// Unlock Account
		String user_unlock_script = "powershell.exe  \"E:\\OneDrive\\Work\\PowerShell\\PasswordPortal\\unlock_account.ps1 " + user_email + " " + user_email + "\" ";
		...
		BufferedReader user_unlock_output = new BufferedReader(new InputStreamReader(user_unlock_process.getInputStream()));
		...
		// Get User's Detail
		String user_detail_result_script = "powershell.exe  \"E:\\OneDrive\\Work\\PowerShell\\PasswordPortal\\user_detail_result.ps1 " + user_email + "\" ";
		...
		// Pass user email to user.jsp
		request.setAttribute("login_email", user_email);
		request.getRequestDispatcher("user.jsp").forward(request, response);
		
		// Pass UserUnlock.java to LoginCheck.java

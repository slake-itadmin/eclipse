<%@ page import="com.b2tech.FirstServlet"%>
...
.unlock_account_container input[type=text] {
	position: absolute;
	width: 300px;
	height: 34px;
	left: 775px;
...
.unlock_account_container p.reset input[type=submit] {
	position: absolute;
	padding: 0 18px;
	width: 100px;
	height: 29px;
...
.unlock_account_container p.unlock input[type=submit]:active, .unlock_account_container p.reset input[type=submit]:active {
	background: #cde5ef;
...
%>
	<marquee behavior="scroll" width="100%" direction="left" bgcolor=green
		behavior="scroll" direction="left" scrollamount="11">
		Welcome
		<%=userDetails.get(0)%>
	...
	<p class="user_detail">&nbsp;User Detail</p>	
		...
		<p class="unlock_account">&nbsp;Unlock Account</p>
		...
		<input type="text" name="input_email_address" placeholder="Enter your email address">
		...
		<p class="unlock_account_result"><label for="unlock_account_result">Result &nbsp;&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp; <%=request.getAttribute("unlockAccountResult")%></label></p>
		</div>
	</div>
